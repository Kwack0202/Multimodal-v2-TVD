...
for model in model_types:
    for ticker in tickers:
        for day in days:
            df = pd.read_csv(f"./csv/6_Buy_Sell_signal/full_period/{model}/{day}/{ticker}.csv", index_col=0)
            ...
            if not os.path.exists(f'./csv/7_Backtesting/full_period/{model}/{day}/'):
                os.makedirs(f'./csv/7_Backtesting/full_period/{model}/{day}/')
                
            merged_df.to_csv(f'./csv/7_Backtesting/full_period/{model}/{day}/{ticker}.csv', index=True)