...
for ticker in tickers:
    for day in days:
        for model in model_types:
            ...
            # 데이터 프레임 Load
            df = pd.read_csv(f'./csv/7_Backtesting/full_period/{model}/{day}/{ticker}.csv', index_col=0)
            ...
            if not os.path.exists(f'./csv/8_Backtesting_Final/full_period/'):
                    os.makedirs(f'./csv/8_Backtesting_Final/full_period/')
                    
            df.to_csv(f"./csv/8_Backtesting_Final/full_period/{ticker}_{model}_{day}.csv", encoding='utf-8-sig')
    ...
    summary_df = pd.DataFrame(summary_data, columns=['Data_Frame_Name', 'No_Trade', 'Winning_Ratio', 'Profit_Average', 'Loss_Average', 'Payoff_Ratio', 'Profit_Factor',
                                                     'MDD', 'MDD(portfolio)', 'MDD_return', 'MDD_return(portfolio)','Max_Holding_Period', 'Mean_Holding_Period', 
                                                     'Max_Profit', 'Max_Profit_Return', 'Max_Loss', 'Max_Loss_Return',
                                                     'Max_Cum_Profit', 'Max_Cum_Profit_Return', 'Max_Cum_Loss', 'Max_Cum_Loss_Return', 
                                                     'Last_Cumulative', 'last_Cum_PER',
    ...
    summary_df.to_csv("./csv/result_summary/individual/Final_results_Summary_full_period.csv", encoding='utf-8-sig')
