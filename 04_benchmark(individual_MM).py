...
class SingleStockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ta = torch.tensor(self.data['TA_X'].iloc[idx], dtype=torch.float)
        img = Image.open(self.data['img_X'].iloc[idx]).convert('RGB')
        if self.transform:
            img = self.transform(img)
        label = torch.tensor(self.data['Y'].iloc[idx], dtype=torch.float)
        return ta, img, label
...
class StockPredictor(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(self.hidden_unit, self.mlp_hidden_unit // 2)  # Feature fusion layer
        self.bn1 = nn.BatchNorm1d(self.mlp_hidden_unit // 2)      # Batch normalization
        self.fc2 = nn.Linear(self.mlp_hidden_unit // 2, 1)  # Final prediction layer 
        self.dropout = nn.Dropout(p=self.dropout_prob)
        
        # Activation function
        self.relu = nn.ReLU()
    
    def forward(self, ta, img):
        ta, _ = self.lstm(ta)
        ta = self.fc_ts(ta[:, -1, :])  # (batch_size, hidden_size)

        vit_out = self.vit(img).last_hidden_state[:, 0, :]  # (batch_size, hidden_size)
    ...
    def get_model_name(self):
        model_name = "Benchmark_MM_single_"
        ...
        model_name += f"(MLP_{self.hidden_unit}_{self.mlp_hidden_unit // 2})"
...
def train_model(model, train_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        correct = 0
        total = 0
        for ta, img, labels in train_loader:
            ta = ta.to(device)
            img = img.to(device)
            labels = labels.to(device)
            
            optimizer.zero_grad()
            outputs = model(ta, img).squeeze(1)
...
def test_model(model, test_loader, criterion, device):
    ...
    with torch.no_grad():  # 그라디언트 계산 비활성화
        for ta, img, labels in test_loader:
            # 데이터를 디바이스로 이동
            ta = ta.to(device)
            img = img.to(device)
            labels = labels.to(device)
...
# 데이터 로드 및 처리
def load_and_process_data(day, ticker, window_length):
    data_path = f'./csv/3_Multimodal_data/v2_25/{day}/{ticker}_{window_length}_TA_img.pkl'
    data = pd.read_pickle(data_path)

    split_num = 753

    train_data = data.iloc[:-split_num]
    test_data = data.iloc[-split_num:]
    ...
    for window_length in window_lengths:
        for ticker in tickers:
            for day in days:
                ...
                # 데이터 로드 및 처리
                train_data, test_data = load_and_process_data(day, ticker, window_length)
                train_dataset = SingleStockDataset(train_data, transform=transform)
                test_dataset = SingleStockDataset(test_data, transform=transform)
                ...
                save_model(model, f'./saved_model/benchmark_MM_single_{window_length}/{day}/{ticker}.pth')

                # 모델 로드 및 평가
                loaded_model = StockPredictor().to(device)
                loaded_model.load_state_dict(torch.load(f'./saved_model/benchmark_MM_single_{window_length}/{day}/{ticker}.pth'))
                ...
                os.makedirs(f'./csv/4_model_results/benchmark_MM_single_{window_length}/{day}/', exist_ok=True)
                results_df.to_csv(f'./csv/4_model_results/benchmark_MM_single_{window_length}/{day}/{ticker}.csv', index=False)